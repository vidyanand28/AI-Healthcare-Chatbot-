...
from config import DIETS_CSV, MODEL_PATH, PRECAUTIONS_CSV
...
class Predictor:
    def __init__(self) -> None:
        self._precautions = pd.read_csv(PRECAUTIONS_CSV)
        self._diets = pd.read_csv(DIETS_CSV)
        with open(MODEL_PATH, "rb") as f:
            self._svc = pickle.load(f)

    def predict(self, patient_symptoms: list[str]) -> str:
        input_vector = np.zeros(len(SYMPTOMS_DICT))
        for symptom in patient_symptoms:
            if symptom not in SYMPTOMS_DICT:
                raise ValueError(f"Symptom '{symptom}' not recognized.")
            input_vector[SYMPTOMS_DICT[symptom]] = 1
        return DISEASES_LIST[self._svc.predict([input_vector])[0]]

    def get_recommendations(self, disease: str) -> tuple[list, list]:
        pre = self._precautions[self._precautions['Disease'] == disease][
            ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']
        ]
        die = self._diets[self._diets['Disease'] == disease]['Diet']
        return list(pre.values), list(die.values)
